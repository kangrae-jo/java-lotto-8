...
| Money | 금액 연산을 담당하는 값 (상금, 구매 금액 등에서 사용) |
...
- [x] 로또 기계(`LottoMachine`)는 로또(`Lotto`)를 발행한다.
    - [x] 판매원(`Cashier`)은 사용자가 입력한 금액(`Price`)을 확인하고, 그 금액으로 티켓(`Ticket`)을 발급한다.
    - [x] 티켓(`Ticket`)은 구매 가능한 로또(`Lotto`)의 개수를 나타내는 값 객체이다.
    - [x] 로또 기계(`LottoMachine`)는 발급된 티켓(`Ticket`)을 이용해 해당 개수만큼의 로또(`Lotto`)를 생성한다.
